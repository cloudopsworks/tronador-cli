...
The supported installation sources are published GitHub Release archives from
...
- Scripts allow prerelease installation only when the user explicitly passes a
  version/tag.
- Homebrew and Chocolatey package metadata is generated by the GoReleaser
  pipeline for stable release jobs, using the same archive names and checksums
  as the GitHub Release.

## Maintainer workflow

1. Keep `.goreleaser.yml` as the source of truth for release archives,
   Homebrew, Chocolatey, checksums, signatures, and installer uploads.
